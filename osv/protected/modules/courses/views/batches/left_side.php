...
<div class="emp_cont_left">
    ...
    <div class="status_bx">
    	<ul>
        	<li style="border-right:1px #D4D4D4 solid"><span><?php echo count(Students::model()->findAll("batch_id=:x", array(':x'=>$_REQUEST['id']))); ?></span>Students</li>
            <li style="border-left:1px #fff solid;border-right:1px #D4D4D4 solid;"><span><?php echo count(Subjects::model()->findAll("batch_id=:x", array(':x'=>$_REQUEST['id']))); ?></span>Subjects</li>
            <li style="border-left:1px #fff solid"><span><?php echo count(TimetableEntries::model()->findAll(array('condition'=>'batch_id=:x', 'group'=>'employee_id','params'=>array(':x'=>$_REQUEST['id'])))); ?></span>Employees</li>
        </ul>
     <div class="clear"></div>
    </div>
	<div class="namelist">
    	<ul>
        	<li><strong>Class Teacher :</strong> 
    ...
    <?php $time='nt_red'; ?>
    	...
    	<span>Notice</span>
        No Batch weekdays are defined.Default Settings Selected.<br />
        <?php echo CHtml::link('Define Now', array('/courses/weekdays','id'=>$_REQUEST['id']));?></div>
        <?php } ?>
    	...
    	<span>Notice</span>
        No Batch Class Timings are defined.<br />
        <?php echo CHtml::link('Define Now', array('#'),array('id'=>'add_class-timings-side'));?></div>
        <?php } ?>
    	...
    	<span>Notice</span>
        Time Table Not Created.<br />
        <?php echo CHtml::link('Create Now', array('weekdays/timetable','id'=>$_REQUEST['id']));?></div>
        <?php } ?>
        ...
        <div class="notifications nt_red">
    	<span>Notice</span>
        No Subjects Added.<br />
        <?php echo CHtml::link('Add Now', array('#'),array('id'=>'add_subjects-side')) ?></div>
        <?php } ?>
    ...
    <li><?php echo CHtml::link('Add Student', array('/students/students/create'),array('class'=>'student')) ?></li>
   <?php /*?> <li><?php echo CHtml::link('Add Event', array('#'),array('id'=>'add_events-side','class'=>'addevnt')) ?></li><?php */?>
    <li><?php echo CHtml::link('New Subject', array('#'),array('id'=>'add_subject-name-side','class'=>'newsub')) ?></li>
    <li><?php echo CHtml::link('Mark Attendance', array('/courses/studentAttentance','id'=>$_REQUEST['id']),array('class'=>'mark')) ?></li>
    
   <?php /*?> <li><a class="copy" href="#">Copy Batch Settings<span class="active"></span></a></li><?php */?>
    <li><?php echo CHtml::link('Promote Batch', array('batches/promote','id'=>$_REQUEST['id']),array('class'=>'promote')) ?></li>
    <?php if($batch->is_active=='1')
	{?>
    <li><?php echo CHtml::link('Deactivate Batch', array('batches/deactivate','id'=>$_REQUEST['id']),array('confirm'=>'Are You Sure,Deactivate This Batch ?','class'=>'deactivate last')) ?></li><?php }
	else
	{ ?>
    <li><?php echo CHtml::link('Activate Batch', array('batches/activate','id'=>$_REQUEST['id']),array('confirm'=>'Are You Sure,Activate This Batch ?','class'=>'deactivate last')) ?></li><?php }?>

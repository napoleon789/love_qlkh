...
//                            Yii::trace('$script:'.$script);
				...
				$user->salt = $salt;
		        $user->password = User::model()->hashPassword($password,$salt);
    ...
    public function actionStep4() {
        ...
        if (isset($_POST['RegisterForm'])) {
            ...
            if ($model->validate() === true) {
                ...
                $ch = curl_init();
                $data = http_build_query(CMap::mergeArray($model->attributes, array('ip'=>gethostbyname($_SERVER['SERVER_NAME']),'domain'=>$_SERVER[ 'SERVER_NAME'  ],'key'=>Yii::app()->session['key'])), null, '&');

...
<h1>FinanceFeeCategories </h1>
<script language="javascript">
function batch()
{
 var id = document.getElementById('batchdrop').value;
 window.location ='index.php?r=financeFeeCategories/index&FinanceFeeCategories[id]='+id;	
}
</script>
...
$del_title=Yii::t('admin_finance-fee-categories', 'Delete Confirmation');
 ...
 $cancel=Yii::t('admin_finance-fee-categories', 'Cancel');
   ?>
<?php
if(isset($_REQUEST['FinanceFeeCategories']))
 {
 ...
 //VIEW
    ...
    $('.fan_view').each(function(index) {
        ...
        $(this).bind('click', function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/finance-fee-categories/returnView",
                ...
                success: function(data) {
                    $.fancybox(data,
                            {    "transitionIn"    :  "elastic",
                                ...
                                $.fn.yiiGridView.update('finance-fee-categories-grid', {url:'<?php echo Yii::app()->request->baseUrl;?>/index.php?r=financeFeeCategories/index&FinanceFeeCategories[id]=<?php echo $_REQUEST['FinanceFeeCategories']['id'];?>',data:{"FinanceFeeCategories_page":page}});
                                }//onclosed
                            });//fancybox
                    //  console.log(data);
                } //success
...
// DELETE
    ...
    $('.fan_del').each(function(index) {
        ...
        deletes[id] = function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/index.php?r=financeFeeCategories/ajax_delete",
                data:{"id":id,"YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"},
                    beforeSend : function() {
                    $("#finance-fee-categories-grid").addClass("ajax-sending");
                },
